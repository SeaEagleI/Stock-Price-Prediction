...
from layers import stacked_dense, stacked_lstm
...
def model_lstm(time_steps, data_dim1, data_dim2, num_classes, method='category'):
    ...
    lstm1 = LSTM(data_dim1 * 8, return_sequences=True)(input1)
    lstm1 = LSTM(data_dim1 * 4)(lstm1)
    lstm1 = Dense(data_dim1 * 4)(lstm1)
    ...
    x = stacked_dense(mux, (data_dim1 + data_dim2) * 4, 2)
    if method == 'category':
...
class LSTMA(StrategyBase):
    ...
    def fit_model(self, class_weight=None):
        ...
        self.model.fit(
            ...
            epochs=200, verbose=0, validation_split=0.05, batch_size=50,

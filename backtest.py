...
rate_list = []  # 持有期收益率
buydate_list = [
    '2020-04-01', '2020-05-06', '2020-06-01', '2020-07-01', '2020-08-03',
    '2020-09-01', '2020-10-09', '2020-11-02'
]  # 买入交易日
selldate_list = [
    '2020-04-30', '2020-05-29', '2020-06-30', '2020-07-31', '2020-08-31',
    '2020-09-30', '2020-10-30', '2020-11-27'
]  # 卖出交易日
...
def buy(stockcode, date, number, price):
    ...
    if (remaining > allCost):
        ...
        print("买入了代码为" + str(stockcode) + '的股票，价格为：' + str(price) + '，股数为：' +
              str(number) + '，可用头寸为：' + str(round(remaining, 2)))
...
def sell(stockcode, date, number, price):
    ...
    print("卖出了代码为" + str(stockcode) + '的股票，价格为：' + str(price) + '，股数为：' +
          str(number) + '，可用头寸为：' + str(round(remaining, 2)))
    # print('持仓情况为：', hold_stocks)
    ...
    for date in date_list:
        if date in buydate_list:  # 买入日
            ...
            for key in hold_stocks.keys():
                ...
                tmp = tmp.reset_index(drop=False)
                try:
                    trading(key, date, hold_stocks[key], tmp['close'].iloc[0],
                            's')  # 交易日停市
                except:
